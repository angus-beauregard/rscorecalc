...
from ocr_engine import extract_courses_from_image  # your existing stub

EXCEL_RSCORE_FILE = "R-Score Calculator (perfect).xlsx"


# ---------- helpers copied in here so it's self-contained ----------
...
def rank_courses_to_improve(df: pd.DataFrame, bump: int = 5) -> pd.DataFrame:
    base = compute_overall_rscore(df)
    rows = []
    for idx, row in df.iterrows():
        tmp = df.copy()
        improved_mark = min(row["mark"] + bump, 100)
        tmp.loc[idx, "rscore"] = compute_rscore_excel(
            improved_mark, row["group_avg"], row["group_sd"]
        )
...
def normalize_col_name(raw: str) -> str:
    ...
    s = " ".join(s.split())
    if s in ["class", "classe", "course", "course name", "cours", "subject"]:
        return "course_name"
    if s in ["grade", "mark", "note", "result", "score"]:
        return "mark"
    if s in ["average", "avg", "group avg", "class average", "moyenne", "moyenne de groupe"]:
        return "group_avg"
    if s in ["standard", "std", "sd", "standard deviation", "ecart type", "écart type", "group sd", "group std"]:
        return "group_sd"
    if s in ["credit", "credits", "cr", "unites", "unités", "units"]:
        return "credits"
    return s
...
def coerce_csv_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"CSV is missing these columns (after normalization): {missing}")
...
def show_dashboard():
    # init shared state defaults (in case user came here directly)
    ...
    with right:
        ...
        with col_sem:
            ...
            school = st.selectbox(
                ...
                index=list(semester_end_dates.keys()).index(st.session_state["selected_school"])
                if st.session_state["selected_school"] in semester_end_dates
                else 0,
    ...
    with left:
        ...
        with tabs[1]:
            ...
            st.caption("We accept: class, grade, average, standard, credits (any order, any case).")
        ...
        with tabs[2]:
            ...
            with st.form("manual_form"):
                ...
                gavg = st.number_input("Group average (%)", 0.0, 100.0, 75.0)
                gsd = st.number_input("Group SD", 0.0, 30.0, 8.0)
        ...
        with tabs[3]:
            ...
            st.write(
                "We simulate improving each course by a few points and show which one gives you the largest increase in your **overall** R-score."
            )
        ...
        with tabs[4]:
            st.markdown("#### Goals")
            st.write("Set your target R-score. The overview will reflect this.")
            new_target = st.number_input(
                "Target R-score",
                min_value=0.0,
                max_value=50.0,
                value=st.session_state["target_r"],
                step=0.1,
                key="target_input_goals",
            )
            st.session_state["target_r"] = new_target
            st.info("Your overview on the right uses this target.")
